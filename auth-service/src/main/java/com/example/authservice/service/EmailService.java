...
public class EmailService {
    ...
    public void sendPasswordResetCode(String toEmail, String code) {
        try {
            MimeMessage mimeMessage = createMimeMessage(
                    toEmail,
                    "[LinkFolio] 비밀번호 재설정 인증 코드",
                    createHtmlPasswordResetEmail(code) // 비밀번호 재설정용 템플릿
            );
            javaMailSender.send(mimeMessage);
            log.info("{}로 비밀번호 재설정 코드 발송 성공: {}", toEmail, code);
        } catch (Exception e) {
            log.error("{}로 비밀번호 재설정 코드 발송 실패", toEmail, e);
            throw new RuntimeException("메일 발송 중 오류가 발생했습니다.", e);
        }
    }
    ...
    // 공통 로직 추출
    private MimeMessage createMimeMessage(String toEmail, String subject, String htmlBody) throws Exception {
        MimeMessage mimeMessage = javaMailSender.createMimeMessage();
        MimeMessageHelper helper = new MimeMessageHelper(
                mimeMessage,
                true, // 멀티파트
        ...
        helper.setText(htmlBody, true); // HTML 활성화
    ...
    // 비밀번호 재설정 템플릿
    private String createHtmlPasswordResetEmail(String code) {
        return "<html>"
                + "<body style='font-family: Arial, sans-serif; margin: 40px;'>"
                + "  <div style='border: 1px solid #ddd; padding: 20px; max-width: 600px; margin: auto;'>"
                + "    <h2 style='color: #333;'>LinkFolio 비밀번호 재설정</h2>"
                + "    <p>비밀번호 재설정을 위해 아래 인증 코드를 5분 이내에 입력해주세요.</p>"
                + "    <div style='background-color: #f4f4f4; padding: 15px; text-align: center; font-size: 24px; font-weight: bold; letter-spacing: 3px; margin: 20px 0;'>"
                + code
                + "    </div>"
                + createEmailFooter()
    ...
    private String createEmailFooter() {
        return "    <hr style='border: 0; border-top: 1px solid #eee; margin-top: 20px;'>"
