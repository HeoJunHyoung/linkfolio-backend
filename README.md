...
### 서비스 구성 및 상세 문서
각 서비스에 대한 상세한 아키텍처와 구현 내용은 아래 개발자 가이드를 참고해 주세요.
...
|  | 오류율 (%) | 0.36 | 0.00 | **완전 제거** |
| **Portfolio Service** | 평균 응답시간 (ms) | 210.08 | 385.60 | **+83.5% 증가 (악화)** |
|  | 처리량 (TPS) | 15.81 | 14.56 | **-7.8% 감소** |
|  | 오류율 (%) | 58.62 | 0.04 | **-99.9% 감소 (사실상 제거)** |
| **Community Service** | 평균 응답시간 (ms) | 1796.24 | 656.58 | **-63.4% 감소** |
|  | 처리량 (TPS) | 12.13 | 12.62 | **+4.0% 증가** |
|  | 오류율 (%) | 12.13 | 0.00 | **완전 제거** |
| **Support Service** | 평균 응답시간 (ms) | 2579.43 | 1043.60 | **-59.5% 감소** |
|  | 처리량 (TPS) | 12.65 | 22.04 | **+74.2% 증가** |
|  | 오류율 (%) | 0.00 | 0.00 | 변화 없음 |

> 💡 **Portfolio Service 성능 지표 해석**
> * 응답 시간과 처리량이 다소 악화된 것처럼 보이나, 이는 **대규모 트래픽 상황에서의 안정성 확보(오류율 58% → 0%)를 위한 구조적 개선(Split Caching)** 결과입니다. 

### 상세 분석 보고서

각 서비스별 부하 테스트 시나리오, 병목 구간 분석 및 구체적인 튜닝 과정은 아래 문서에서 확인하실 수 있습니다.
...
개발 과정에서 마주친 주요 기술적 난관과 이를 해결한 과정은 아래 트러블 슈팅 문서를 참고해주세요.
